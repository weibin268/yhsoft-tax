...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  ,thisTable = function(){
    ...
    // 把当前的对象也放出去以便在table.reload的时候能够使用定义这个这个类里面的方法
    id && (thisTable.config[id] = options, thisTable.that[id] = this);
  ...
  table.dataParams = ['url', 'method', 'where', 'contentType', 'headers', 'parseData', 'request', 'response', 'data', 'page'/*, 'limit', 'limits'*/];

  // //表格重载 支持用户强制指定重载方式
  // table.reload = function(id, options, request){
  //   var that = this;
  //   var config = thisTable.config[id];
  //   // 参数添加了一个校验，更加灵活，如果没有条件的改变只是request当前页面的话可以直接在第二个传布尔值过来
